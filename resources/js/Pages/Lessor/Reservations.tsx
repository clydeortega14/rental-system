...
const statusBadge = (status: string) => {
  ...
  const badges: Record<string, { className: string; icon: React.ReactNode; label: string }> = {
    RESERVED: { className: "bg-green-100 text-green-700", icon: <CheckCircle className="w-4 h-4" />, label: "Confirmed" },
    PENDING: { className: "bg-yellow-100 text-yellow-800", icon: <Clock className="w-4 h-4" />, label: "Pending" },
    CANCELLED: { className: "bg-red-100 text-red-700", icon: <XCircle className="w-4 h-4" />, label: "Rejected" },
  };

  const badge = badges[status] || { className: "bg-gray-100 text-gray-600", icon: null, label: status };
  return <span className={`${base} ${badge.className}`}>{badge.icon} {badge.label}</span>;
};
...
function Reservations({ lessorReservations }: ReservationsProps) {
  ...
  const handleAction = () => {
    ...
    router.put(`/lessor/property-reserve/${selectedRes.id}/status`, { status }, {
      onSuccess: () => {
        toast({ title: `Reservation ${status === "RESERVED" ? "confirmed" : "rejected"}`, description: `Booking for ${selectedRes.guestName} is now ${status}.` });
        closeDialog();
      },
      onError: () => {
    ...
    <div className="max-w-8xl mx-auto p-6 space-y-6">
      ...
      <header className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <h1 className="flex items-center text-3xl font-bold mb-6 text-orange-600">
          <BiCalendarCheck className="w-6 h-6 text-orange-500 mr-2" /> Reservation Bookings
        </h1>
        <select
      ...
      {filteredReservations.length === 0 ? (
        ...
        <div className="space-y-4">
          {filteredReservations.map((res) => (
            <Card key={res.id} className="flex flex-col sm:flex-row gap-4 sm:gap-6 p-4 rounded-xl hover:shadow-lg">
              <img src={res.imageUrl} alt={res.property} className="w-full sm:w-32 h-40 sm:h-24 rounded-md object-cover" />
              <div className="flex-1 space-y-1">
                <h2 className="text-lg font-semibold text-gray-900">{res.property}</h2>
                <p className="text-gray-700">Guest: {res.guestName}</p>
                <p className="text-gray-600 text-sm">Acquire: {new Date(res.acquire).toLocaleDateString()}</p>
                <p className="text-gray-600 text-sm">Return: {new Date(res.return).toLocaleDateString()}</p>
                {res.hasConflict && <div className="mt-1 text-sm text-red-600 font-semibold">⚠ Double Booking Detected</div>}
              </div>
              <div className="flex sm:flex-col sm:items-end justify-between sm:justify-center gap-2 sm:gap-3">
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end">
                    <DropdownMenuItem onClick={() => setViewingRes(res)}>View Details</DropdownMenuItem>
                    {res.status === "PENDING" && (
                      <>
                        <DropdownMenuItem onClick={() => { setSelectedRes(res); setActionType("reject"); }}>Reject</DropdownMenuItem>
                        <DropdownMenuItem onClick={() => { setSelectedRes(res); setActionType("confirm"); }}>Confirm</DropdownMenuItem>
                      </>
                    )}
                  </DropdownMenuContent>
                </DropdownMenu>
      ...
      <Dialog open={!!viewingRes} onOpenChange={() => setViewingRes(null)}>
        <DialogContent className="w-full max-w-md sm:max-w-lg max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>Booking details for {viewingRes?.guestName}</DialogDescription>
          </DialogHeader>
