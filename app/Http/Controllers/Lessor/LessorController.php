...
class LessorController extends Controller
{
    public function dashboard()
    {
        ...
        $totalIncome = Booking::whereHas('rentalListing', fn($q) => $q->where('user_id', auth()->id()))
            ->where('status', 2) // assuming 2 = reserved/completed
            ->sum('total_cost');

        $monthlyIncome = Booking::whereHas('rentalListing', fn($q) => $q->where('user_id', auth()->id()))
            ->where('status', 2)
        ...
        $upcoming = Booking::with(['bookedBy', 'rentalListing'])
            ->whereHas('rentalListing', fn($q) => $q->where('user_id', auth()->id()))
